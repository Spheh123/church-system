...
test('Ministry permissions isolate ushers, restrict notes, protect attendance and balance assignment',async()=>{
 ...
 for(const file of ['schema','upgrade','usher-role','ministry-operations','pastoral-notes','automatic-assignment','export-permissions','visitor-intake-journey','contact-milestones'])await db.exec(fs.readFileSync('supabase/'+file+'.sql','utf8').replace('create extension if not exists pgcrypto;',''));
 ...
 await as(3,`select save_attendance('${service}',1,current_date,'Sunday first','',11,12,3,4)`);
 await assert.rejects(as(3,`select save_attendance('${service}',1,current_date,'Sunday first','',12,12,3,4)`),/Someone changed/);
 await assert.rejects(as(2,"select save_attendance(null,null,current_date,'Wednesday','',1,1,1,1)"),/Attendance access/);
 assert.equal((await as(1,'select * from service_attendance')).rows[0].men,11);
