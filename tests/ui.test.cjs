...
for (const [page,role] of [['dashboard','admin'],['dashboard','pastor'],['people','team'],['person','admin'],['followup','team'],['reports','pastor'],['attendance','usher'],['ministry','pastor']]) {
  test(`${page} renders real page nodes and controls for ${role}`, async () => {
    ...
    if(page==='dashboard') {
      ...
      assert.equal(document.querySelector('.admin-panel').classList.contains('hidden'),role==='pastor');
    }
    if(page==='people') {
    ...
    if(page==='person') {
      assert.equal(document.getElementById('assignedToSelect').value,'11111111-1111-4111-8111-111111111111');
