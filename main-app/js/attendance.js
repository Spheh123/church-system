...
$('attendanceForm').onsubmit=async event=>{event.preventDefault();event.submitter.disabled=true;try{
 ...
 }catch(error){setMessage($('attendanceMessage'),/duplicate key/.test(error.message)?'This service already has an entry. Find it below and choose Correct counts.':error.message,'error');}finally{event.submitter.disabled=false;}};
function render(){
...
async function load(){records=await readAllRows('service_attendance','*','service_date');render();}
for(const id of ['trendFrom','trendTo','trendService','trendMetric','trendGroup'])$(id).onchange=render;
initProtectedPage({allowedRoles:['admin','pastor','usher'],onReady:async()=>{await load();subscribeTables(['service_attendance'],load);}});
